<form action="{{route('dtr.id', [$id])}}" method="get" target="_blank">
{{-- <form action="{{route('id.single')}}/{{$id}}/monthly/{{date('m', strtotime('month this month'))}}/{{date('Y')}}" method="get" target="_blank"> --}}
    <button type="submit" data-id="{{$id}}" class="btn btn-primary"  data-placement="left" data-tt="tooltip" title="Check DTR"><i class="fas fa-eye"></i></button>
</form>
